...
public class InvertedIndexGenerator {
	...
	public void generateInvertedIndex() {
		...
		for (File file : listOfFiles) {
			if (file.isFile()) {
				...
				createWORDIndex("OutputCACMFiles" + "/" + file.getName(),
						file.getName());
			}
		}
		writeHashMap();
		...
		writeNumOfTokensPerDoc();
	}
	
	private void writeNumOfTokensPerDoc(){
		PrintWriter writer;
		File file = new File(resultPath+"numOfTokensPerDoc.txt");
		try {
			writer = new PrintWriter(file, "UTF-8");
			for (Map.Entry<Integer, Integer> entry : numberOfOneGramTokensPerDocument
					.entrySet()) {
	...
	private void writeHashMapToFile(String filename, Map<String, Integer> hashMap){
		...
		File rankFile = new File(resultPath+"FreqAgainstRank_"+filename);
		try {
	...
	private void writeOneGramHashMap(){
		File hashMap = new File(resultPath+"invertedIndexOneGram.txt");
		PrintWriter writer;
		try {
	...
	private void writeBiGramHashMap(){
		File hashMap = new File(resultPath+"invertedIndexBiGrams.txt");
		PrintWriter writer;
		try {
			writer = new PrintWriter(hashMap, "UTF-8");
			for (Map.Entry<String, List<Index>> entry : invertedIndexBiGrams
	...
	private void writeTriGramHashMap(){
		File hashMap = new File(resultPath+"invertedIndexTriGrams.txt");
		PrintWriter writer;
		try {
			writer = new PrintWriter(hashMap, "UTF-8");
			for (Map.Entry<String, List<Index>> entry : invertedIndexTriGrams
